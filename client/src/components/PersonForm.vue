<template>
    ...
    <Form @submit="nextStep">
        <template #fields>
            <Input label="Nome" type="text" v-model="form.name" :error="errorMessage.name" />
            <template v-if="form.personType == PERSON_TYPES.PF.code">
                <Input label="CPF" type="text" mask="###.###.###-##" v-model="form.cpf" :error="errorMessage.cpf" />
                <Input label="Data de nascimento" type="date" v-model="form.birthDate" :error="errorMessage.birthDate" />
            </template>
            <template v-else>
                <Input label="CNPJ" type="text" mask="##.###.###/####-##" v-model="form.cnpj" :error="errorMessage.cnpj" />
                <Input label="Data de abertura" type="date" v-model="form.openingDate" :error="errorMessage.openingDate" />
            </template>
            <Input 
                label="Telefone" 
                type="text" 
...
<script setup>
    ...
    import { isValidCPF, isValidCNPJ, isValidBirthDate, isValidOpeningDate, isValidNameLength, isValidPhone } from '@shared/validators.js';
    ...
    const title = computed(() => {
        return props.form.personType == PERSON_TYPES.PF.code ? 'Pessoa Física' : 'Pessoa Jurídica';
    });
    ...
    const isDisabled = computed(() => {
        if (props.form.personType == PERSON_TYPES.PF.code) {
            return !props.form.name.length || 
                (!props.form.cpf.length || !props.form.birthDate.length) ||
                !props.form.phone.length;
        } else {
            return !props.form.name.length || 
                (!props.form.cnpj.length || !props.form.openingDate.length) ||
                !props.form.phone.length;
        }
    });
    ...
    function nextStep() {
        ...
        const isErrorEmpty = Object.values(errorMessage.value).every((error) => !error);
        if (!isErrorEmpty) {
            return;
        }
